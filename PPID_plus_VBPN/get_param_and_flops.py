...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        hrms = net._modules["RNet"](torch.cat((upms, pan), 1), None).detach()
    ...
    for param in params:
        ps_params += param
    if ps_params // 10 ** 6 > 0:
        vbpn_params = str(round(ps_params / 10 ** 6, 2)) + 'M'
    elif ps_params // 10 ** 3:
        ps_params = str(round(ps_params / 10 ** 3, 2)) + 'k'
    vbpn_paramps_paramss = str(ps_params)
    for flop in flops:
        ps_flops += flop
    for mem in memory:
        ps_mem += mem
